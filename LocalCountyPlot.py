...
def setDefaultPlotData():
    data = {'title': 'Visited US Counties',
            'refresh_info': None,
            'epsg_format': None #Set this in the plot to define it at the very end
            }
    return data

def formatPlotText(text):
    return text
    text = text.replace('&', '\&')
    output_string = '\n'.join(' '.join(local_list[i:i+20]) for i in range(0, len(local_list), 20))
    return output_string
# Filter the counties
def plotCounties(county_map, county_list, plot_data = setDefaultPlotData()):
    ...
    if not plt.get_fignums(): #If figure does not yet exist, load the shape file and initialize the plot
        ...
        county_map.to_crs(plot_data['epsg_format']).plot(ax=ax, color='lightgrey', alpha=0.5)  # Plot all counties for an outline
    else:
        fig = plt.gcf()
        ax = fig.gca()
    ...
    if plot_data['epsg_format'] is None:
        plot_data['epsg_format'] = str(county_map.crs).replace("EPSG:","") #If no projection defined, default to layout listed as "CRS" in the SHP file
        
    state_county_fips = county_map['STATEFP']+county_map['COUNTYFP']
    visited_counties = county_map[state_county_fips.isin(county_list)]
    visited_counties = visited_counties.to_crs(epsg=plot_data['epsg_format'])
    ...
    if plot_data['refresh_info'] is not None:
        if plt.gca().texts:
            plt.gca().texts[0].remove()
        plt.text(0.01, 0.5, 'States Visited:\n' + formatPlotText(plot_data['refresh_info']), transform=plt.gca().transAxes, verticalalignment='center')
    plt.draw()
    plt.pause(0.01)
